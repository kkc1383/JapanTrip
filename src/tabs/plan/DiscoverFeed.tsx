...
export default function DiscoverFeed() {
  ...
  const filtered = tag === '전체' ? posts : posts.filter(([, p]) => p.tag === tag)
  ...
  return (
    <div>
      ...
      <div className="mb-2.5 flex gap-1.5 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {TAGS.map(t => {
          ...
          return (
            ...
            >
              {t} <span className={active ? 'opacity-80' : 'opacity-60'}>{count}</span>
            </button>
          )
        })}
      </div>
      ...
      {hasMore ? (
        <div ref={sentinelRef} className="py-4 text-center text-[11px] tracking-wide text-sub">
          불러오는 중...
        </div>
      ) : filtered.length > 0 ? (
        <p className="py-3 text-center text-[10px] tracking-[0.3em] text-sub/60">· 피드의 끝 ·</p>
      ) : (
